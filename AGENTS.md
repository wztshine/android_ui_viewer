...
- **After capture**: `tree_display_id` set to captured `display_id` while `last_tree_display_id` is left stale, so the ui() re-parse guard re-parses the multi-display XML for the captured display (prevents showing `file_displays[0]`'s tree under the wrong label)
