...
- **uiautomator2 捕获** — 通过 atx-agent JSON-RPC 截图 + 层级 dump
...
3. **U2 捕获** — 连接设备后点击 `U2`，通过 atx-agent 捕获（需安装 uiautomator2 相关工具）
4. **多屏** — 工具栏下拉选择显示 ID
5. **检查** — 在截图上悬停高亮元素，点击选中（同位置点击循环上溯父级）
6. **交互** — 双击发送 tap 指令，拖拽发送 swipe 指令
7. **浏览树** — 点击箭头展开/折叠，点击标签选中，自动滚动到选中节点
8. **属性** — 在右侧属性面板查看选中元素的属性（分隔栏可拖拽调整宽度）
9. **保存** — 点击 `Save` 导出当前截图和 XML
...
- Rust edition 2021 及以上
- 已安装 ADB 并加入 PATH（设备交互需要）
- OpenGL 3.2+（或 Vulkan/DirectX 12 via wgpu）
- 已连接的 Android 设备并开启 USB 调试（ADB / U2 捕获需要）
- **U2 捕获**：设备上需运行 atx-agent。最简单：
  `python -m uiautomator2 init`（自动推送 atx-agent + app-uiautomator.apk）。
  手动：将 `atx-agent` 推送至 `/data/local/tmp/atx-agent`，
  `app-uiautomator.apk` 推送至 `/data/local/tmp/app-uiautomator.apk`。
...
本仓库代码全部由 **DeepSeek V4 Flash** 完成，耗时约 8 小时。如果控件树解析有问题，
或者有其他问题，可以把控件树文件和相关 bug 告诉 AI，让它修复。
